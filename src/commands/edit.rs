...
const NEW_LINE_NAME: &str = "[New raw line]";
const RAW_NAME: &str = "[Raw]:";
const DELETE_OPTION_NAME: &str = "[Delete]";
...
fn edit_interactive(path: Option<&str>, id: Option<&str>,) -> Result<(), Error> {
    ...
    loop {
        ...
        match Window::new("Edit Entry").lines(lines.len() as i32).format('s').show(lines.clone()) {
            Ok(s) => {
...
fn get_menu_action(s: String) -> EditMenuAction {
    if s.starts_with("path: ") { EditMenuAction::EditPath }
    else if s.starts_with("uuid: ") { EditMenuAction::EditUuid }
    else if s.starts_with("username: ") { EditMenuAction::EditUsername }
    else if s.starts_with("password: ") { EditMenuAction::EditPassword }
    else if s.starts_with("url: ") { EditMenuAction::EditUrl }
    else if s == NEW_LINE_NAME { EditMenuAction::AddOther }
    else if s == DELETE_OPTION_NAME { EditMenuAction::Delete }
    else if s == RAW_NAME { EditMenuAction::DoNothing }
